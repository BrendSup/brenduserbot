...
async def seslimuzik(event):
    ...
    async with bot.conversation(chat) as conv:
        try:     
            ...
            await event.reply(f"` deyəsən` {chat} `bloklamısan. Zəhmət olmasa bloku açıb yenidən yoxla.`")
            return
        ...
        if sesler.audio:
            try:
                ...
                titlemsg = "Ses"           
            await event.edit(f"__{titlemsg} tapıldı, yüklənir...")
            indir = await sesler.download_media()
            await event.edit(f"__{titlemsg} adlı səsi yüklədim, səsli mesaj olaraq göndərirəm...")
            voice = await asyncio.create_subprocess_shell(f"ffmpeg -i '{indir}' -y -c:a libopus 'unvoice.ogg'")
            await voice.communicate()
            if path.isfile("unvoice.ogg"):
                await event.client.send_file(event.chat_id, file="unvoice.ogg", voice_note=True, caption=caption, reply_to=rep_msg)
                ...
                await event.edit("`Tapılan səs səsli mesaja çevirə bilmədim!`")
            remove(indir)
        ...
        elif sesler.buttons[0][0].text == "Nəticə Yoxdur":
            ...
            await event.edit(f"__{titlemsg} `yüklədim.` **İnşAllah doğrudur :)**. `Səsli mesaj olaraq göndərirəm...`__")
...
Help.add_info('(Sonuna  {n} yazsanız səsli mesajı "@BrendUserBot ilə yükləndi" yazısını qaldırar.)')
Help.add()
